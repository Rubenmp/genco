...
use crate::domain::core::file_system::file_creator::{create_file_if_not_exist, create_file_with_content};
...
impl UserInput {
    ...
    pub(crate) fn request_missing_user_input(&mut self) {
        for (variable_id, input_variable) in self.get_variables_mut() {
            ...
            let user_input = cli_query::ask_input(format!("Please, insert the content for variable \"{}\":", variable_id_str).as_str());
...
impl FileInputVariable {
    ...
    pub fn get_variable_id(&self) -> Vec<u8> {
        ...
        let end_index = self.content_pattern.clone().len() - get_end_variable_pattern().as_bytes().len();
...
fn get_variables(file: &PathBuf) -> HashMap<Vec<u8>, UserInputVariable> {
    ...
    let file_content = fs::read_to_string(file).expect(format!("Error reading file {}", file.to_string_lossy()).as_ref());
...
fn find_next_variable(file_content: &String, initial_index: usize) -> Option<(usize, usize)> {
    if let Some(start_index) = file_content[initial_index..].find(get_start_variable_pattern()).map(|i| i + initial_index) {
        let end_variable_pattern = get_end_variable_pattern();
        if let Some(end_index) = file_content[start_index..].find(end_variable_pattern).map(|i| i + start_index) {
