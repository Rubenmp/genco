...
use crate::domain::core::parser::string_helper::escape_str_for_json;
...
pub trait ParserNode {
    ...
    fn get_content(&self) -> String {
        let file = File::open(self.get_file_path()).expect("Parser node file must open.");
        let mut buffer = get_content_from(file, self.get_start_byte(), self.get_end_byte());
    ...
    fn get_content_bytes_with_previous_empty_space(&self) -> String {
        ...
        let mut buffer = get_content_from(file, current_start_byte, self.get_end_byte());
