...
pub fn assert_same_as_file(expect_result_file_path: &Path, result: String) {
    let data =
        fs::read_to_string(expect_result_file_path).expect("Unable to read expected result file");
...
pub fn assert_same_file(expect_result_file: &PathBuf, actual_result_file: &PathBuf) {
    let expected_data =
        fs::read_to_string(expect_result_file).expect("Unable to read expected result file");
    let actual_data =
        fs::read_to_string(actual_result_file).expect("Unable to read expected result file");
...
pub fn assert_file_is(result: &Path, expected_file: &str) {
    ...
    if let Some(file_str) = result.iter().map(|i| i.to_str()).last() {
        ...
        assert!(false, "Expected \"{}\" file not found.", expected_file);
    }
}
