...
impl ParserNode for JsonNode {
    fn new(file_path: &Path) -> Self {
        ...
        let file_content = fs::read_to_string(file_path_str)
            .unwrap_or_else(|_| panic!("File path \"{}\" should exists to parse json node", file_path_str));
    ...
    fn is_printable(&self) -> bool {
        if let Some(node_type) = &self.node_type {
            return matches!(node_type, JsonNodeType::QuotationMark | JsonNodeType::String | JsonNodeType::Comma | JsonNodeType::Number
            | JsonNodeType::LBrace | JsonNodeType::RBrace | JsonNodeType::Colon | JsonNodeType::LBracket | JsonNodeType::RBracket
            | JsonNodeType::Null);
        }
        false
    }
