...
pub fn parse(json_file_path: &PathBuf) -> Vec<AvroItem> {
    let json_root_node = json::parser::json_parser::parse(json_file_path);
...
mod tests {
    ...
    use crate::domain::usecase::avro::parser::avro_parser::parse;
    use crate::domain::usecase::avro::parser::dto::avro_item::AvroItem;
    use crate::domain::usecase::avro::parser::dto::avro_item_type::AvroItemType;

    #[test]
    fn parse_basic() {
        let test_name = "avro.avsc";
        let file_path = get_test_file_path(get_current_file_path(), test_name);

        let avro_items = parse(&file_path);
